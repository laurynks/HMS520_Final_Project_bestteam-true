...
stgpr_to_dismod <- function(input_stgpr_bundle){
  ...
  representative_type_converter <- function(val){
    ...
    representative_map <- data.frame(id = -1:11, type = as.character(representative_type))
  ...
  source_type_converter <- function(val){
    ...
    source_type_long = c("DSP before 1990", "Vital Registration", "Sample Registrations System", "Disease surveillance points",
                         ...
                         "other")
    source_map = data.frame(id = 0:26, type_short = as.character(source_type_short), type_long = as.character(source_type_long))
    if (val %in% source_map$id){
      return(source_map$type[source_map$id == val])
    } else {
      return("Not set")
    }
  }
