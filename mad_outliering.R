###########################################################
### Author: Mae Dirac, modified by Hannah Han, Jaimie Adelson, and Shaun Roberts
...
temp_dir <- paste0(j_root, "temp/srob982/")
date <- Sys.Date()
date <- gsub("-", "_", date)
...
path_to_input_data <-  paste0("/snfs1/WORK/12_bundle/imp_gbs/278/03_review/02_upload/", date, "_", decomp_step, "_",bundle_id, "_sex_split_NID_less_50_outlier", ".xlsx")
path_to_output_data <- paste0("/snfs1/WORK/12_bundle/imp_gbs/278/03_review/02_upload/", date, "_", decomp_step, "_",bundle_id, "MAD1", ".xlsx")
description <- "2020 step2 sex split data, outliered MAD = 2"
...
## INPUT DATA
#dt <- as.data.table(read.xlsx(path_to_input_data))
#dt <- get_crosswalk_version(crosswalk_version)
dt <- get_bundle_version(22562, fetch = "all")
...
# #Fixing seqs
# dt_inp[ ,crosswalk_parent_seq := seq]
# dt_inp[ ,seq := as.character(seq)]
# dt_inp[ ,seq := ""]

#Make sure so SE's are above 1. If there is an SE above 1, If os is above 1 turn to NULL.
#stopifnot(sum(review_sheet_final$standard_error > 1, na.rm = T) == 0)
